...
Design intelligence for interfaces, and game development intelligence for the simulation
underneath them.

Installs **two** skills:

- **vishwakarma** — interfaces on web, Android and Apple. One entry in the Skills panel, 33
  reference files and 3 verification scripts behind it, loaded on the branch you are actually on.
- **vishwakarma-studios** — games. Engines, the fixed timestep, ECS, physics, animation,
  rendering, audio, game feel, input, netcode, game AI, design loops, performance, production,
  accessibility and shipping, across 68 references each under a 6,000-token budget.

They divide by layer rather than by subject. A game's launcher, settings screen and store page
are app UI and belong to `vishwakarma`; the loop, the physics and the netcode belong to
`vishwakarma-studios`.
