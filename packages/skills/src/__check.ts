...
import { estimateTokens, skillCost, validateManifest } from './manifest.js'
...
console.log(
  'body tokens',
  estimateTokens(accessibleComponents.content.body),
  'chars',
  accessibleComponents.content.body.length,
)
for (const r of accessibleComponents.content.references ?? [])
  console.log('ref', r.id, estimateTokens(r.content ?? ''))
