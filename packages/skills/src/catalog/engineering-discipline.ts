...
/**
 * The characteristic failure of a capable agent is not incompetence. It is confident motion
 * in an unverified direction.
 *
 * An agent picks one reading of an ambiguous request and runs with it, builds machinery for
 * requirements nobody has yet, edits lines nobody asked about, and reports success against a
 * goal it never made checkable. What makes this class of failure hard to catch is that each
 * instance looks like good practice from the inside: the over-abstracted version follows
 * recognised design patterns, the tidied diff follows the style guide, the confident plan
 * reads like competence. They are wrong on *timing*, not on taste — the right idea produced
 * before the evidence that would justify it.
 *
 * This skill is the always-on layer, and it is deliberately small: five principles that turn
 * motion into verified progress, plus the clause that stops them from turning a typo fix into
 * a clarification interview. Everything else — debugging method, planning method, diff
 * hygiene — sits in references that load when the situation calls for them.
 */
export const engineeringDiscipline: SkillManifest = {
  vsm: '1.0',
  ...
  license: 'Apache-2.0',
  category: 'workflow',
  tags: ['discipline', 'debugging', 'planning', 'verification', 'scope', 'baselines', 'process'],

  activation: {
    always: true,
    intents: [
      'the user gives a request that admits more than one reasonable reading',
      'the user asks for something to be made faster, cleaner, better, or more accessible without naming a threshold',
      'starting implementation on a task of any size, before the first edit is written',
      'a bug has been reported and a fix is about to be attempted',
      'a hypothesis about a failure has been disconfirmed and another is about to be formed',
      'a change is finished and the diff is about to be reported as complete',
      'an abstraction, extension point, configuration flag, or interface is about to be introduced',
      'the requirements as stated conflict with each other or with the existing code',
      'a plan is being written for work that will run unsupervised for several steps',
    ],
    keywords: [
      'make it faster',
      'clean this up',
  ...
  content: {
    summary:
      'Convert motion into verified progress: enumerate ambiguous readings with costs before choosing, measure before changing, restate the task as a checkable condition, keep every changed line traceable to the request, and abstract only on the second real case.',

    body: `# Engineering Discipline
    ...
    references: [
      {
        ...
        title: 'Debugging: reproduction, hypothesis discipline, and bisection',
        answers:
          'How do I debug a failure without guessing — reproducing it on demand, forming hypotheses that can be disconfirmed, bisecting, instrumenting rather than inferring, and telling a symptom from a cause?',
...
- Can the causal chain from root cause to observed symptom be stated in one sentence, or was a symptom suppressed without a mechanism?`,
      },
      {
        ...
        title: 'Planning: costed menus, baselines, checkable steps, and slices',
        answers:
          'How do I turn an ambiguous request into a plan — costed readings, a measured baseline, checkable success conditions, step-to-verification pairs, real data inspected first, and independently verifiable slices?',
        content: `# Planning and decomposition
...
- Was the level of ceremony matched to reversibility, blast radius, cost of error and genuine ambiguity — with low-stakes changes handled directly rather than escalated into clarification?`,
      ...
      {
        id: 'scope-and-diff-discipline',
        title: 'Scope: what belongs in the diff, and what does not',
        answers:
          'What am I allowed to change beyond what was asked — cleanup, formatting, local conventions, dead code, abstraction timing — and what does an oversized solution tell me?',
        content: `# Scope and diff discipline
...
- Does the new code match local quote style, naming, typing and error idioms, with any disagreement raised as its own proposal rather than applied unilaterally?`,
  ...
  rules: [
    {
      ...
      verifiedBy: 'evidence-before-motion',
    },
    {
      id: 'engineering-discipline/baseline-before-change',
      strength: 'must',
      statement:
        'Report the current value of any metric you propose to improve, with the conditions it was measured under, before making the change.',
      evidence: {
        rationale:
          'Without a starting number "better" is unfalsifiable: the work has no defined end, and a change that improves the unmeasured number while worsening the measured one is indistinguishable from success. A baseline is also the only way a regression in the untouched direction becomes visible in the same breath as the improvement.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'Lazy-load three routes, note that the main chunk dropped 180KB, report a smaller bundle.',
        good: 'Main chunk 512KB gzipped, time-to-interactive 400ms on the second page (throttled 4G, cold cache). After lazy-loading: main chunk 332KB, second-page TTI 1.9s. The bundle win costs 1.5s on navigation — worth it only if first load dominates.',
      },
      exceptions: [
    ...
    {
      id: 'engineering-discipline/restate-as-checkable',
      strength: 'must',
      statement:
        'Restate the task as an observable condition — named inputs, named outputs, named thresholds — before writing any code, and report the inability to do so as the finding.',
      evidence: {
        rationale:
          'A crisp success condition is what makes unsupervised iteration possible: an agent that can check its own output loops until the condition holds, while an agent with a vague goal must return to a human after every step to ask whether this counts as done. The restatement is also where a misunderstanding surfaces, at the point where it costs a sentence rather than a rebuild.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'Task: "add validation". Proceed to add validation.',
        good: 'Task: "add validation" means empty strings and whitespace-only strings are rejected with FIELD_REQUIRED, addresses without an @ with FIELD_FORMAT, both server-side, proven by four unit tests plus one integration test on the endpoint. Confirm before I start?',
      },
      verifiedBy: 'evidence-before-motion',
    },
    {
      id: 'engineering-discipline/step-carries-verification',
      strength: 'must',
      statement:
        'Write each plan step paired with the check that proves it worked — naming an input and an expected result — before execution starts, not after the code exists.',
      evidence: {
        rationale:
          'Verification written after the fact is written to pass: once the code exists the check you invent is shaped by what the code does, so you pick the input that works and the path you happened to exercise. Writing it first shapes it by the requirement instead. A step whose check is "it should work" is not specific enough to fail, so it will not.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'Step: add rate limiting. Verification: confirm rate limiting works.',
        good: 'Step: rate-limit at 10 req/min per key. Verification: curl the endpoint eleven times within a minute; the eleventh returns 429 with a Retry-After header, and 200 health checks in the same minute all return 200.',
      },
      verifiedBy: 'evidence-before-motion',
    },
    {
      id: 'engineering-discipline/reproduce-first',
      strength: 'must',
      statement:
        'Reproduce a reported failure on demand before modifying any code in response to it.',
      evidence: {
        rationale:
          'A fix applied to an unreproduced bug cannot be verified — the symptom may resolve for an unrelated reason, or may never have had the cause assumed. A test that fails for the reported reason is the only evidence that the later green result means anything.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'Bug report says sorting is wrong -> edit the comparator -> ship.',
        good: 'Bug report says sorting is wrong -> write a failing test that reproduces the exact ordering -> confirm it fails for that reason -> fix -> confirm green -> confirm existing tests still pass.',
      },
      verifiedBy: 'diagnosis-quality',
    },
    {
      id: 'engineering-discipline/one-change-per-observation',
      strength: 'must',
      statement:
        'Pair every hypothesis with the observation that would disconfirm it, and change exactly one thing between observations.',
      evidence: {
        rationale:
          'Confirming evidence is available for almost any theory in a large enough system, so a hypothesis with no named disconfirmation is unfalsifiable in practice. Changing two things at once destroys the information the run would have produced: when the failure stops you have learned nothing about it, and you have shipped one change you do not need, which then sits in the codebase as an unexplained constant nobody dares remove.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'The request sometimes times out, so raise the timeout and add a retry. It stopped failing.',
        good: 'Hypothesis: the cache entry is stale. Disconfirmation: log the key write timestamp; if it is newer than the last mutation, the cache is not the problem. Change nothing else until that log has been read.',
      },
      verifiedBy: 'diagnosis-quality',
    },
    {
      id: 'engineering-discipline/escalate-after-three-guesses',
      strength: 'should',
      statement:
        'After three disconfirmed hypotheses, stop guessing from the same model — read the dependency source, capture a trace, or re-examine whether the component you are debugging is the broken one.',
      evidence: {
        rationale:
    ...
    {
      id: 'engineering-discipline/abstraction-on-second-case',
      strength: 'should-not',
      statement:
        'Do not introduce an extension point, strategy object, configuration flag, injected collaborator, or plugin registry until a second real caller exists in the codebase with its own requirements visible.',
      evidence: {
        rationale:
          'With one instance in hand nothing distinguishes the essential from the incidental, so an abstraction guessed from it encodes that example’s accidents as the general case. The second real case then does not fit and you pay twice — once to build the seam in the wrong place, once to move it, with the move complicated by whatever came to depend on it meanwhile.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'Three fixed-percentage discount codes arrive as a DiscountStrategy interface, an AbstractDiscount base, three implementations, a registry, a factory, and a config file — 200 lines across seven files.',
        good: 'A six-line function with a three-entry lookup table. When the second and third discount types arrive, the variation turns out to live in eligibility rules rather than arithmetic, and the seam goes there.',
      },
      verifiedBy: 'proportionality',
    },
    {
      id: 'engineering-discipline/diff-traces-to-request',
      strength: 'must-not',
      statement:
        'Do not include reformatting, added annotations, rewritten comments, renamed locals, or improved adjacent logic in a diff that was requested for something else; offer them as separate changes instead.',
      evidence: {
        rationale:
          'Three costs, each sufficient alone. A twelve-line diff is read line by line and a two-hundred-line diff of which twelve matter is skimmed, so cosmetic noise is functionally a way of avoiding review. Blame stops pointing at the reason a line exists, severing the trail for whoever investigates an incident later. And a "cosmetic" restructure occasionally changes behaviour — reordered imports change initialisation order, a rewritten conditional changes short-circuit evaluation — in the part of the diff nobody is examining.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'One-line null-check fix, plus the formatter running on save across the file, plus type hints added to four neighbouring functions.',
        good: 'One-line null-check fix. "While in this file I noticed parseConfig swallows the original exception — happy to fix that separately."',
      },
      verifiedBy: 'change-discipline',
    },
    {
      id: 'engineering-discipline/leave-foreign-dead-code',
      strength: 'must-not',
      statement:
        'Do not delete pre-existing dead code as a side effect of another task; remove only what your own change orphaned, and report the rest by name and location.',
      evidence: {
        rationale:
          'Ownership of the mess determines the permission to touch it: code your change orphaned you understand completely, while code that was already unreferenced you do not. "Unreferenced" is weaker than it looks — a symbol may be called reflectively, by name from configuration, from a template, from a queued job, from an annual migration, or from another repository. It is the highest-variance action available for the lowest possible reward.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'While fixing the export bug, also delete legacyExportHandler because nothing references it.',
        good: 'Fix the export bug. Report: "legacyExportHandler in reports.ts has no references I can find and appears unused since the v3 migration — worth deleting separately if someone can confirm the batch jobs do not call it."',
      },
      verifiedBy: 'change-discipline',
    },
    {
      id: 'engineering-discipline/match-local-conventions',
      strength: 'should',
      statement:
        'Match the surrounding quote style, import ordering, naming, typing, error idioms and test structure, even where you disagree with them, and raise a disagreement as its own proposal.',
      evidence: {
        rationale:
          'A convention’s value is almost entirely in its uniformity rather than its content, so a locally inconsistent improvement is a net loss even when the improvement is real. Reading code is pattern matching, and one function written to different conventions costs the reader an interruption that carries no information: they stop to work out whether the difference is meaningful, and it is not.',
        confidence: 'strong',
      },
      exceptions: [
    ...
    {
      id: 'engineering-discipline/stop-when-confused',
      strength: 'must',
      statement:
        'Stop and state precisely what does not make sense — including any conflict between two requirements, or between a requirement and the code — rather than reconciling it silently and continuing.',
      evidence: {
        rationale:
          'Work built on a misunderstanding does not need correcting, it needs discarding: the misunderstanding sits upstream of every later decision, so the structure that grew from it is shaped wrong throughout. Silent reconciliation is the worst available option because the person never learns their requirements conflict, and the resulting behaviour matches neither thing they asked for.',
        confidence: 'established',
      },
      examples: {
        language: 'text',
        bad: 'I am confused about the archive endpoint. (Or worse: pick the reading that makes both statements approximately true and build it.)',
        good: 'The spec says the archive endpoint is idempotent, but also that each call appends an audit row — those cannot both hold unless the audit row is exempt from the idempotency guarantee. Which is it?',
      },
      verifiedBy: 'evidence-before-motion',
    },
  ],
  ...
  verification: [
    ...
    },
    {
      ...
      kind: 'self-review',
      description: 'Confirm a fix rests on an observed cause rather than a suppressed symptom.',
      blocking: true,
      questions: [
    ...
    },
    {
      ...
      kind: 'self-review',
      description: 'Confirm the diff contains what was asked for and nothing else.',
      blocking: true,
      questions: [
  ...
  relatedSkills: ['design-review', 'code-quality', 'ui-generation-workflow'],
}
